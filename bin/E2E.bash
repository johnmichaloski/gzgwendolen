...
cmd+=( --tab  --working-directory="$r" -e 'bash -c "printf \"\e]2;crclapp\a\";export LD_LIBRARY_PATH=/opt/ros/kinetic/lib:$libs:$LD_LIBRARY_PATH; echo $LD_LIBRARY_PATH; source /opt/ros/kinetic/setup.bash; source $root/devel/setup.bash; sleep 30s; cd $crclapp; ./crclapp -r fanuc_;exec bash"')

cmd+=( --tab  --working-directory="$q" -e 'bash -c "printf \"\e]2;telnet\a\";source /opt/ros/kinetic/setup.bash;source $HOME/src/gzaprsros-xenial/devel/setup.bash;source $root/devel/setup.bash; sleep 40s; telnet 127.0.0.1 64444 ;exec bash"')
