                                       ...
                                       CCanonWorldModel instatus)
{
    ...
    if (incmd.crclcommandnum != _lastcmdnum)
    {
        ...
        if(0!=_goRobot->InitPose(r_lastpose,
                                 ...
                                 _nc->rotationalParams()))
        {
            std::cerr << "go init pose failed goal pose= "<< dumpPoseSimple(r_goalpose) << "\n";
        }
    ...
    {
        std::cerr << "IK failed next pose= " << dumpPoseSimple(r_nextpose)<< "\n";
                                               ...
                                               CCanonWorldModel instatus)
{
    ...
    if (incmd.crclcommandnum != _lastcmdnum)
    {
        ...
        {
            std::cerr << "Error: bad parameter names - no leading action\n";
            return CanonStatusType::CANON_ERROR;
        }
        ...
        {
            std::cerr <<"Unknown gripper action paramter setting\n";
        }
    }
    _lastcmdnum=-1;
    return CanonStatusType::CANON_DONE;
                                 ...
                                 CCanonWorldModel instatus)
{
    ...
    {
        ROS_DEBUG_STREAM( "Exception in  GoInterpreter::ParseCommand() thread: " << e.what() );
