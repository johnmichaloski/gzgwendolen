...
static void cleanup()
{
    ...
    // Stopping application - this is in case ^C pressed
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        try {
            ...
            Globals.sRosPackageName = RCS::robotconfig.getSymbolValue<std::string>("system.RosPackageName","gzrosrcs");
            ...
            fprintf(stderr, "gzrosrcs: Compiled %s %s\n" , __DATE__ , __TIME__ );
            fprintf(stderr, "gzrosrcs: Build %d\n"  , BUILD);
            fprintf(stderr, "gzrosrcs: Started %s\n" , Globals.getTimeStamp().c_str()) ;
            ...
            for (size_t i = 0; i < robots.size(); i++)
            {
                ...
                std::vector<std::string> v = { "LD_LIBRARY_PATH","gzrosrcs_LIBRARY_PATH" };
    ...
    {
        std::cerr << Globals.strFormat("%s%s%s", "Abnormal exception end to gzrosrcs", e.c_str() , Globals.getTimeStamp().c_str());
        cleanup();
    }
    catch (std::exception e)
    {
        std::cerr << Globals.strFormat("%s%s%s", "Abnormal exception end to gzrosrcst", e.what(), Globals.getTimeStamp().c_str());
        cleanup();
    }
    catch (...)
    {
        std::cerr << "Abnormal exception end to  gzrosrcs at" <<  Globals.getTimeStamp().c_str();
        cleanup();
    }
    std::cout << "gzrosrcs: Stopped " << Globals.getTimeStamp().c_str() << "\n" ;
}
