...
public:
    ...
    /// \brief AddController add controller to command line interpreter.
    /// Really only handles one controller, but in theory could handle multiple NCs.
    /// \param nc pointer to controller.
    ///
    void addController(std::shared_ptr<CController>  _nc);
private:
