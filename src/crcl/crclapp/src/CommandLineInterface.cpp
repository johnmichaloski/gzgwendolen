...
bool CComandLineInterface::jog(char c,double amt, int &jnt, int &axis)
{
    ...
    curpos.position = crclApi->_status.r_curjnts.position; // Globals.crclApi->_status.robotjoints.position;
    double incr=0;
    ...
    {
        nextpos=curpos;
        tf::Pose r_curpose=crclApi->_status.r_curpose;
        ...
        try{
            // seed joints
            joints.position=subset(crclApi->_status.r_curjnts.position,
                                   rcs_robot.numJoints());
...
int CComandLineInterface::interpretLine(std::string line)
{
    ...
    else if (msg.compare("auto") == 0)
    {
        ...
        //
        // FIXME: use crclAPI to set joint speeds...
        //        Globals.setGripperJointSpeeds(Globals.currentGripperJointSpeed()*.5);
        //        Globals.setGripperJointSpeeds(Globals.currentGripperJointSpeed()*.5);
    ...
    else if (msg.compare( 0, strlen("echo"), "echo") == 0)
    {
        ...
        else
        {
            ...
            if(bCrcl && bCmd)
            {
                Globals.DEBUG_Crcl_Command=1;
                thread_sleep(1000);
                Globals.DEBUG_Crcl_Command=0;
            }
            if(bCrcl && bStatus)
            {
                Globals.DEBUG_Crcl_Status=1;
                thread_sleep(10000);
                Globals.DEBUG_Crcl_Status=0;
            }
            if(bRos && bCmd)
            {
                Globals.DEBUG_Ros_Command=1;
                thread_sleep(1000);
                Globals.DEBUG_Ros_Command=0;
            }
            if(bRos && bStatus)
            {
                Globals.DEBUG_Ros_Status=1;
                std::this_thread::sleep_for(std::chrono::milliseconds(2000));
                Globals.DEBUG_Ros_Status=0;
            }
            if(bModel && bStatus)
            {
                Globals.DEBUG_Model_Status=1;
                thread_sleep(1000);
                Globals.DEBUG_Model_Status=0;
            }
        }
    }
    else if (msg.compare( 0, strlen("debug"), "debug") == 0)
    {
        ...
        else
        {
            ...
            if(bCrcl && bCmd)
                Globals.DEBUG_Crcl_Command=bOn;

            if(bCrcl && bStatus )
                Globals.DEBUG_Crcl_Status=bOn;

            if(bRos && bCmd)
                Globals.DEBUG_Ros_Command=bOn;

            if(bRos && bStatus)
                Globals. DEBUG_Ros_Status=bOn;

            if(bModel && bStatus)
                Globals.DEBUG_Model_Status=bOn;
    ...
    {
        std::cout << "speed:\n\trobot joint vel=" << crclApi->_status.currentRobotJointSpeed()
                  << "speed:\n\tjgripper    vel=" <<  crclApi->_status.currentGripperJointSpeed()
                  << "\n\tlinear            vel="  <<  crclApi->_status.currentLinearSpeed()
                  << "\n\trotational vel=       "  <<  crclApi->_status.currentAngularSpeed()
                  << "\n" << std::flush;
    }
    else if (msg.compare( 0, strlen("gripper "),"gripper ") == 0)
    {
        ...
        if(msg == "slower")
        {
            crclApi->_cmd.setGripperJointSpeeds(.5);
            //          Globals.setGripperJointSpeeds(Globals.currentGripperJointSpeed()*.5);
        }
        else if(msg == "faster")
        {
            crclApi->_cmd.setGripperJointSpeeds(2.);
            //Globals.setGripperJointSpeeds(Globals.currentGripperJointSpeed()*2.);
        }
        else if(msg == "reset")
        {
            crclApi->_cmd.setGripperJointSpeeds(1.);
    ...
    else if (msg.compare( 0, strlen("record "), "record ") == 0 )
    {
        ...
        {
            recordFile << rcs_robot.robotPrefix() <<  msg << "\n";
            std::vector<double> joints = crclApi->_status.r_curjnts.position;
            recordFile << "\tJoints   =" << vectorDump(joints, ",", "%6.3f") << "\n" ;
            tf::Pose r_curpose=crclApi->_status.r_curpose;
            //Globals.robotKinematics()->FK(joints, r_curpose);
